...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ocultar o mostrar el menú inferior dependiendo del destino actual
        navController.addOnDestinationChangedListener { _, destination, _ ->
            when (destination.id) {
                R.id.loginFragment -> {
                    // Si estamos en el LoginFragment, ocultar el menú inferior
                    binding.bottomNavigationView.visibility = View.GONE
                }
                else -> {
                    // En otros fragmentos, mostrar el menú inferior
